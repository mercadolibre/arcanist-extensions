...
final class MavenUnitTestEngine extends ArcanistUnitTestEngine {
    ...
    final protected function findTestXmlFiles() {
        ...
        $regex = new RegexIterator($iterator, '/^.+target\/surefire-reports\/TEST.*\.xml$/i', RecursiveRegexIterator::GET_MATCH);
        ...
        if (!count($files)) {
            throw new ArcanistUsageException('Could not find any test output files. '
    ...
    final protected function parseReport($report) {
        ...
        $results = $this->parser->parseTestResults(Filesystem::readFile($report));
        return $results;
    }
